...
export const RichTextBlock = ({ data }: RichTextBlockProps) => {
  ...
  return (
    <div className="w-full bg-bg-contrast py-28">
      <div className="container mx-auto px-4 md:px-8 lg:px-16 max-w-7xl">
        <div className="pb-8">
          {heading && (
            <h1 className="text-3xl sm:text-5xl font-bold mb-4 text-[var(--color-bg-light)]">
              {heading}
            </h1>
          )}
          {subheading && (
            <p className="text-lg sm:text-xl mb-6 text-[var(--color-primary)]">
              {subheading}
            </p>
          )}
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 space-y-4 sm:space-y-0">
          {features?.map((f, idx) => {
            const hasIcon =
